...
## Loading Resources
The process of loading resources is much the same as for loading a new scene, with the notable exception that more than one resource can be loaded at a time, and that the resource must be checked and retrieved by path.  
-Use "SceneManager.StartResourceLoad("RESOURCE PATH TO LOAD")" to initialize the load  
-Use "SceneManager.CheckResourceLoadStatus("RESOURCE PATH TO LOAD") to check the load status in a process function. When this function returns 1.0, the resource is loaded and ready to instantiate. If it returns -1.0, the load failed  
-Once the above function returns 1.0, use the return value of "SceneManager.FinalizeResourceLoad("RESOURCE PATH TO LOAD")" to retrieve the resource  
...
v0.4: Add resource loading -- Current Version  
